...
func TestRecommendItemsToUser(t *testing.T) {
	...
	resp, err = client.NewRecommendItemsToUser("nonexisting2", 9).SetCascadeCreate(true).SetReqlExpressions(map[string]string{"boolean": "true", "number": "if ('answer' > 0) then 1 else 2", "string": "\"test\""}).Send()
	if err != nil {
		t.Fatal(err)
	}
}
