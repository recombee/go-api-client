...
func TestAddCartAdditionInBatch(t *testing.T) {
	defaultTestSetUp(t)
	...
	var reqs = []requests.Request{
		client.NewAddCartAddition("u_id", "i_id").SetCascadeCreate(true).SetAdditionalData(map[string]interface{}{"answer": 42}),
		client.NewAddCartAddition("entity_id", "entity_id"),
		client.NewAddCartAddition("entity_id", "entity_id").SetTimestamp(func() time.Time { t, _ := time.Parse(time.RFC3339, "2013-10-29T09:38:41.341Z"); return t }()),
		client.NewAddCartAddition("entity_id", "nonex_id"),
		client.NewAddCartAddition("nonex_id", "entity_id"),
		client.NewAddCartAddition("u_id2", "i_id2").SetCascadeCreate(true).SetTimestamp(time.Unix(5, 0)),
		client.NewAddCartAddition("u_id2", "i_id2").SetCascadeCreate(true).SetTimestamp(time.Unix(5, 0)),
	}
