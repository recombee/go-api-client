...
func TestAddBookmarkInBatch(t *testing.T) {
	defaultTestSetUp(t)
	...
	var reqs = []requests.Request{
		client.NewAddBookmark("u_id", "i_id").SetCascadeCreate(true).SetAdditionalData(map[string]interface{}{"answer": 42}),
		client.NewAddBookmark("entity_id", "entity_id"),
		client.NewAddBookmark("entity_id", "entity_id").SetTimestamp(func() time.Time { t, _ := time.Parse(time.RFC3339, "2013-10-29T09:38:41.341Z"); return t }()),
		client.NewAddBookmark("entity_id", "nonex_id"),
		client.NewAddBookmark("nonex_id", "entity_id"),
		client.NewAddBookmark("u_id2", "i_id2").SetCascadeCreate(true).SetTimestamp(time.Unix(5, 0)),
		client.NewAddBookmark("u_id2", "i_id2").SetCascadeCreate(true).SetTimestamp(time.Unix(5, 0)),
	}
