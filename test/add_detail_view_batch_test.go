...
func TestAddDetailViewInBatch(t *testing.T) {
	defaultTestSetUp(t)
	...
	var reqs = []requests.Request{
		client.NewAddDetailView("u_id", "i_id").SetCascadeCreate(true).SetAdditionalData(map[string]interface{}{"answer": 42}),
		client.NewAddDetailView("entity_id", "entity_id"),
		client.NewAddDetailView("entity_id", "entity_id").SetTimestamp(func() time.Time { t, _ := time.Parse(time.RFC3339, "2013-10-29T09:38:41.341Z"); return t }()),
		client.NewAddDetailView("entity_id", "nonex_id"),
		client.NewAddDetailView("nonex_id", "entity_id"),
		client.NewAddDetailView("u_id2", "i_id2").SetCascadeCreate(true).SetTimestamp(time.Unix(5, 0)),
		client.NewAddDetailView("u_id2", "i_id2").SetCascadeCreate(true).SetTimestamp(time.Unix(5, 0)),
	}
